...
| `tools/art/install_gait_walk.py` | (2026-08-28) installs a GAIT-TRANSFER walk batch (green-keyed Codex rows → engine strips): keys #00ff00, rim-band-ONLY despill (a global clamp dulls olive/green garments), TORSO-BAND ANCHORS each frame (gait-donor gens bake canvas travel — 73px drift reads as wobble unless anchored), median-body scale + feet-line pin to the OLD strip's measured geometry, writes mirrors (w/nw/sw←e/ne/se) + flat=s. The severed_thread walk recipe; reuse for warrior/paladin rounds. Then --import + verify_art --stride-min 0.45 + gates. |
