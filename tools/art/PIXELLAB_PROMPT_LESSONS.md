...
| Warlock Hex (`ult`) E f7-8 face WEST (3/4 body, beam + ring fired left; f1-6/f9 clean pure-east) | (v3 flipped the whole climax pose, not just the prop — a plain in-place mirror would keep a face-visible 3/4 body in a pure cardinal, rule 14) | — no regen — **composite: last clean body frame (f6) + the wrong-way frames' FX isolated by COLOUR (purple/white, never a column cut — the hand overlaps the beam base), mirrored and re-seated on the extended hand** (`fix_warlock_ult_e_frames.py`, 2026-08-16) | Body identical to f1-6, authored FX kept, faces east; 0 gens (owner had pre-authorized PixelLab; not needed) |
