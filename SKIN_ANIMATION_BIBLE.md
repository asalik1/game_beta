...
| **Arcane Warlock** | The former green Eldritch Herald identity, retained as an elite. | Preserve its existing visual kit; it is renamed, not part of the current animation pass. | Preserve the existing elite rift language. |
| **Eldritch Warlock** | A reality tailor followed by a four-wing watcher plants living eyes between seams, then pulls victims apart by their threads. | Eyes scan before firing; projectile routes remain taut until contact; curse watchers react only on real DoT ticks. Uses the exact former Void Weaver character art rather than a duplicated sprite. | Condemning the existing circular Void Rift footprint: violet light reveals the chosen area, then countless threads rip vertically upward on the unchanged burst beat. |
...
| **Arcane Warlock — elite** | Former Eldritch Herald. Rename and preserve the existing green visual language; no new work in this pass. |
| **Eldritch Warlock — mythic** | **Locomotion:** exact former Void Weaver character sprite plus one animated familiar made from a central violet eye and exactly four dark-purple wings. **Shadowbolt:** one woven eye forms in a random clear pocket, visibly scans side-to-side, then launches a tiny sharp violet needle from its pupil. The full travelled route persists and collapses with the projectile. **Hex:** a separate two-wing watcher stands above every cursed enemy; on each authoritative DoT tick it recoils and a runtime thread connects its pupil to that enemy—never bake the thread into its sheet. **Dark Pact:** reuse the accepted stitched circle and violent unravel. **Void Rift:** its exact circular ground footprint lights violet; on the existing fourth pull/burst deadline, dense vertical threads erupt across that whole circle. No copied Blink animation and no gameplay-bearing divergence. |
