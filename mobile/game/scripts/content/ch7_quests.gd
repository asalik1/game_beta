...
const SIDE_QUESTS := {
	"ch7_relay_stands": {
		...
		"chapter": "ch7",
		"desc": "Thirty years of relay shifts before her voice went, and Retired Keeper Vasse still mouths her old hour every night. She can't climb to the line's dead stations anymore. Stand a breath at each one, so somebody has.",
		"steps": [
			...
			{"kind": "kill", "target": "storm_harrier", "count": 3, "flag": "sq7_relay_held",
				"text": "Hold the relay line and drive off the storm-harriers at the void's edge"},
		],
	...
	"ch7_void_letter": {
		...
		"chapter": "ch7",
		"desc": "On the void shelf, among the kept things: a sealed letter, addressed in a hand that makes the shard flinch. TO SOMEONE WHO WILL REMEMBER. Elder Maren qualifies. The seal stays unbroken. That's the entire job.",
		"steps": [
	...
	"ch7_korrags_due": {
		"name": "Korrag's Due",
		"chapter": "ch7",
		"desc": "The beast-clans buried the Stormwarden under plains stone and six hundred offerings. His own order never left one. Apprentice Sorrel means to fix that. Carry the relay's shift-token up the downs and leave it with the wolf teeth. But the downs aren't quiet: void-shades circle the cairn, and the dead don't take visitors kindly.",
		"steps": [
...
const QUEST_ITEMS := {
	"ch7_void_letter": {"name": "A Sealed Letter", "icon": "ch7_void_letter", "grade": "C",
		"desc": "Addressed in a hand that makes the shard flinch: TO SOMEONE WHO WILL REMEMBER. The seal is unbroken. Keep it that way. That's the entire job."},
...
const CONVOS := {
	...
	"void_letter_scene": {"cinematic": true, "start": "vl1", "nodes": {
		"vl1": {"who": "Narrator", "cue": "q_void_letter",
			"text": "At the summit camp, Elder Maren turns the sealed letter over once, the wax unbroken, the hand on it long dead, and doesn't open it. Some things are carried, not read. She sets it where the firelight can reach it, for whoever the void meant it for.",
			"next": "vl_fade"},
	...
	"ch7_wander_keeper": {"start": "k1", "nodes": {
		"k1": {"who": "Retired Keeper Vasse",
			...
			"choices": [
				...
				{"text": "\"The line's cut, keeper. Mouthed hours warm nobody, so save the breath for winter.\"",
	...
	"ch7_shrine_vowstone": {"start": "v1", "nodes": {
		...
		"v_slap": {"who": "Narrator", "text": "Palm to stone, one beat, done. Attendance taken in the only ledger on this hill that can't read hearts. The storm doesn't miss a beat for you; storms know the difference, it turns out, between a shift and a signature. Down at the camp an old keeper will hear the station was stood and mouth her hour a little easier tonight, and what she paid thirty years of knees for, you paid a slap. The rounds still happen. The word 'still' is doing new work in that sentence now.", "next": ""},
	...
	"ch7_lore_shelf": {"start": "l1", "nodes": {
		"l1": {"who": "Narrator", "text": "A shelf of not-quite-stone at the tear's edge, and arranged along it, arranged unmistakably, the things the void has caught as they fell out of the world: a child's shoe with a buckle no smith alive could name. A coin from a kingdom absent from every atlas. A letter, sealed, addressed in a hand that makes your shard flinch, to SOMEONE WHO WILL REMEMBER. The void keeps everything, the old warnings say. Standing here, you finally hear the warning's second half, the part the Concord never carved: it keeps everything carefully.",
			"variants": [{"flag": "sq7_letter_taken", "text": "The shelf keeps its careful arrangement: shoe, coin, the letter's slot holding its shape in the dust. The void doesn't seem to mind the borrowing. It seems, if anything, to have been waiting for a courier."}],
			"choices": [
	...
	"ch7_briefing": {"start": "m1", "nodes": {
		"m1": {"who": "Elder Maren",
			...
			"variants": [
				...
				{"flag": "ch7_would_speak", "text": "'Learn the words.' Sorrel said the same thing at nine years old, and the Wardens laughed at her. Nobody on this summit is laughing now. Hold that thought through everything I'm about to tell you."},
				{"flag": "ch7_let_it_end", "text": "You came up here ready to let six centuries lapse and see what the storm owes afterward. Hear the briefing first. Debts this old do not disappear; they change collectors."},
				{"flag": "ch7_hears_wind", "text": "Drake first, sentence later. Practical. Keep listening anyway. On this summit the wind has learned how to turn practical people into punctuation."},
		...
		"m_letter2": {"who": "Narrator", "text": "She takes it like something that might go off, then turns it over, reads the address, and stops. All of her. For three full seconds the summit's one fixed point is an old woman not breathing. 'I know this hand,' she says at last, and doesn't say from where, and you understand that you have just watched sixty years arrive in one envelope. She sets it inside her coat, against the sternum. 'It will be read, bearer. After the stair. Some things you don't open with a storm watching. And your arithmetic was right. I remember everything.'", "next": ""},
	}},
	...
	"ch7_apprentice": {"start": "p1", "nodes": {
		"p1": {"who": "Apprentice Sorrel",
			...
			"variants": [
				{"flag": "sq_paid_ch7_korrags_due", "text": "You left it with him? At the cairn, with the teeth and the feathers? ...Good. Good. Master never spoke of Korrag without standing straighter. I thought that was reverence. I think now it was debt. Four lines and a paid-up cairn, bearer. The order's estate is small, but it's finally honest.", "next": ""},
