...
const SIDE_QUESTS := {
	"ch6_far_shore": {
		...
		"chapter": "ch6",
		"desc": "Fisher Dov hasn't rowed past the leaning reeds since midsummer, and nobody has seen the far shore. Find what the Root keeps of it, then come back and tell him true.",
		"steps": [
	...
	"ch6_kesh_tally": {
		"name": "Kesh's Tally",
		"chapter": "ch6",
		"desc": "The Bloom takes ground faster than the cure-camp's runners can map it. Mark what the green has claimed, the sunken shrine and the cure pool's fence, and bring Kesh the tally. The green does not survey quietly, and bog-lurkers work the same line.",
		"steps": [
...
const CONVOS := {
	...
	"ch6_fisher": {"start": "f1", "nodes": {
		...
		"f_quest": {"who": "Fisher Dov",
			"text": "Since you're bound for the Deep anyway, bearer, here's a fisher's ask. Brekk's croft sat on the far shore. Blue door; he painted it for his wife the spring before she died the first time. Nobody's rowed past the leaning reeds since midsummer, and I can't. Find what's left of the shore. Then come back and tell me true. I've had a bellyful of the bog's kind of giving.",
			"choices": [
	...
	"ch6_briefing": {"start": "b1", "nodes": {
		"b1": {"who": "Deacon Vela",
			...
			"variants": [
				...
				{"flag": "ch6_seeks_truth", "text": "You came for the truth before the miracle. Keep that order. The Deep has swallowed whole pilgrimages by offering the second before anyone asked about the first."},
				{"flag": "ch6_answered_green", "text": "The Deep answered eleven pilgrims this spring, whatever each one asked of it. I have buried what nine of them turned into, and the other two I still hear at night. Whatever you asked at the gate, bearer, unask it."},
				{"flag": "ch6_would_burn", "text": "You mean to answer the green with fire. I won't tell you you're wrong. I will tell you half the people kneeling out there still have names, and I know every one."},
			...
			"next": "b2"},
		"b2": {"who": "Deacon Vela", "text": "We came on pilgrimage. The blight is the land telling the truth: rot comes for everything, and the Choir simply stopped calling that bad news. Sixty years of that creed, bearer, and it held, through plague and Vale and worse. Then we walked into the Deep and found the opposite of rot: growth without death. Things that only add. And half my flock knelt to it on sight.", "next": "b3"},
	...
	"ch6_wildfang": {"start": "k1", "nodes": {
		...
		"k_offer": {"who": "Herbalist Kesh",
			...
			"choices": [
				{"text": "\"I'll carry the tally. Her stakes stopped at the gallery's edge, and someone should push the line.\"",
	...
	"ch6_lore_gallery": {"start": "l1", "nodes": {
		"l1": {"who": "Narrator", "text": "A gallery of white roots, thick as bridge-cables, running through the peat in one direction: down. Every root in the Deep, whatever it feeds above, bends eventually to this bearing. The cure-camp's survey stakes stop at the gallery's edge; the last stake carries Kaethra's own tag, in a steady hand: 'ALL ONE PLANT. STOP CALLING THEM ROOTS. THESE ARE FINGERS.'",
			...
			"choices": [
				...
				{"text": "Leave the west face for a steadier hour. Whatever the Root keeps of the shore, it keeps carefully, and it will keep one day more.",
	...
	"ch6_shrine_schism": {"start": "s1", "nodes": {
		"s1": {"who": "Narrator",
			...
			"choices": [
				...
				{"text": "\"The rot. I've seen the bloom up close, and it's the same hunger with better manners. Your old creed holds.\"",
		...
		"s_after": {"who": "Narrator",
			...
			"choices": [
				{"text": "Set the gate camp's loaf on the table between them, fresh this morning, from the flock that stayed.",
					...
					"resonance": 2.0, "flags": {"sq6_bread_left": true},
					"lose_item": "ch6_gate_loaf", "next": "s_loaf"},
				{"text": "Keep Vela's loaf in your pack. Warm bread buys more on a long road than in a war of doctrines, so let the camps fast on principle a night more.",
					"req_flag": "sq6_bread_taken", "req_not_flag": "sq6_bread_left",
