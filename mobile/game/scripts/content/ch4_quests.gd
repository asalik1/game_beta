...
const SIDE_QUESTS := {
	"out_of_tolerance": {
		...
		"chapter": "ch4",
		"desc": "Overseer Brann can prove the heats climb; he wants one number that goes the other way. Slag left in the Cold Forge goes honestly, completely cold, so chip him a core of it, once you've cleared the slag-brutes that crowd the heap.",
		"steps": [
...
const CONVOS := {
	...
	"ch4_wander_charms": {"start": "w1", "nodes": {
		...
		"w_refund": {"who": "Charm Peddler Nix",
			...
			"variants": [
				...
				{"flag": "ch4_refund_taken", "text": "The pouch, bearer, for the smith at the gate, crew five's leftover. Wages go back to whoever's left to spend them. That's the whole contract.", "next": ""},
			],
			"choices": [
	...
	"ch4_wander_smith": {"start": "w1", "nodes": {
		...
		"w_token": {"who": "Old Smith Harl",
			"text": "Since you're bound for the deep anyway, I hammered something last night. A little iron token, quenched in river water, the old ceremony, the whole hiss. I want it left at that font the maps say not to dig toward. Not as an offering, but as a comparison. Let the Judge weigh one piece of iron that owes it nothing.",
			"variants": [
		...
		"f_more": {"who": "Narrator", "text": "The slag turns over, slow as case law reconsidering itself.",
			...
			"choices": [
				{"text": "Set Harl's water-quenched token on the font's lip, iron that owes the deep nothing, and let the Judge weigh a comparison.",
					...
					"lose_item": "harl_token", "resonance": 3.0,
					"flags": {"ch4_token_left": true}, "next": "f_token"},
				{"text": "Keep Harl's token in your pocket a while longer. Iron that owes the deep nothing, and down here, better it owes you.",
					"req_flag": "ch4_token_taken", "req_not_flag": "ch4_token_left",
