...
const ENEMIES := {
	...
	"stormdrake_veyx": {
		...
		"lore": "It isn't angry. It's a syllable of something that hasn't finished speaking in six hundred years, and it's happy.",
		"mechanics": [
			{"name": "Arc",
			 "tell": "Its signature current lances straight through you, unless you're standing near one of the conductor rods. Those drink the arc instead.",
			 "counter": "Shelter beside a rod to feed it the arc. But every redirect charges that rod, and the third one overloads it into a big blast that destroys it. So rotate between rods instead of overloading one."},
			{"name": "Static Field",
			 "tell": "Static strikes scatter across the ground around you, and they get denser and faster whenever the rods are gone.",
			...
			{"name": "Squall",
			 ...
			 "counter": "The bolts fan out with gaps between them. Read the spacing and slip through. Don't stand still trying to tank it."},
	...
	"stormmouth": {
		...
		"lore": "Six hundred years of speakers kept the vow. He's the first to hear what it was silencing, and agree with it.",
		"mechanics": [
...
static func selftest(game: Node2D) -> String:
	for kind in ["stormdrake_veyx", "unnamed_echo", "stormmouth"]:
		...
		var expected_projectile: String = {
			"stormdrake_veyx": "fx_veyx_windslash",
			"unnamed_echo": "fx_echo_knife",
