...
const SIDE_QUESTS := {
	"oslas_debt": {
		"name": "Osla's Debt",
		"chapter": "ch1",
		"desc": "Tinker Osla owes a dead smith for an axle, and she means to settle. Debts left with the old wood find their owners, her words. The Hollow Oak keeps the wood's accounts.",
		"steps": [
	...
	"hunters_rounds": {
		...
		"chapter": "ch1",
		"desc": "The Old Hunter's legs can't make the full rounds anymore, and a wood where nobody walks the landmarks starts believing nobody's watching it. Cut his sign at Ravine Edge, the Drowned Chapel, and the Collapsed Tower. And the packs he warned about move together now, so thin the ones that shadow the rounds.",
		"steps": [
	...
	"flame_at_window": {
		...
		"chapter": "ch1",
		"desc": "The pilgrim lights a stick of pine at every hearth the blight has touched. It does nothing, the scholars say. Carry one to the Drowned Chapel's altar and see what nothing looks like from the inside.",
		"steps": [
...
const CONVOS := {
	...
	"wander_tinker": {"start": "t1", "nodes": {
		...
		"t_trust": {"who": "Tinker Osla", "text": "There. Lighter already, the cart and me both. Don't spend it, mind. The wood would know. The wood always knows.", "next": ""},
	...
	"wander_pilgrim": {"start": "p1", "nodes": {
		"p1": {"who": "Pilgrim of the Flame",
			...
			"choices": [
				{"text": "\"Cut me a stick, pilgrim. There's a drowned chapel east of here still keeping its candle, and I'll set a flame at its window.\"",
			...
			"next": ""},
		"p_give": {"who": "Pilgrim of the Flame", "text": "You know the chapel? And the candle's still going? Sixty years, and it's still burning. Then take the good one. Pine catches quick and burns honest, and the scholars can explain to a dark house what nothing looks like from inside.", "next": ""},
	}},
	...
	"lore_hollow_oak": {"start": "l1", "nodes": {
		"l1": {"who": "Narrator", "text": "A grandfather oak, split open and hollow. Inside, wax stubs and a child's carved wolf. Someone hid offerings here for the wood's old spirits, long before the blight gave the wood new ones. The candle wax is recent.",
			"choices": [
				...
				{"text": "Keep the pouch riding a while yet. The oak has waited years; it can wait out you.",
					"req_flag": "sq_on_oslas_debt", "req_not_flag": "osla_debt_paid", "next": ""},
			],
			"next": ""},
		"l_debt": {"who": "Narrator", "text": "The pouch settles among the wax stubs and the carved wolf like it was expected. No wind moves, but somewhere above you the old branches shift their grip, the sound a ledger makes when it closes. If the smith walks anywhere still, the wood knows the road better than you do.", "next": ""},
	}},
	...
	"lore_ravine": {"start": "l1", "nodes": {
		"l1": {"who": "Narrator", "text": "The ravine has nothing for you. No monsters, no treasure, no secret door. The view, though. The whole Darkwood rolling east under the mist, and the keep's towers far off, patient as tombstones. You let yourself have one long minute of it.",
			"choices": [
				...
				{"text": "Leave the overlook unmarked for now. The rounds can wait on a sterner hour.",
					"req_flag": "sq_on_hunters_rounds", "req_not_flag": "hunter_mark_ravine", "next": ""},
			],
			"next": ""},
	...
	"lore_drowned_chapel": {"start": "l1", "nodes": {
		...
		"c_watch": {"who": "Narrator", "text": "The wind comes off the water and the flame bends, recovers, bends. Whoever keeps this candle isn't here. Right now, the watch is anyone's.",
			"choices": [
				...
				{"text": "Leave it to its work. It's managed this long without your opinion.",
		...
		"c_sign": {"who": "Narrator", "text": "The wind works the water, and the flame keeps up its old argument with it, sixty years practiced and not losing yet.",
			"choices": [
				{"text": "Cut the hunter's sign above the waterline, where the next walker will look.",
					"req_flag": "sq_on_hunters_rounds", "req_not_flag": "hunter_mark_chapel",
					"flags": {"hunter_mark_chapel": true}, "next": "c_mark"},
	...
	"lore_collapsed_tower": {"start": "l1", "nodes": {
		"l1": {"who": "Narrator", "text": "The watchtower fell the night Vargoth rose, and the masonry still shows the burn-shadow of the guard who stood here when it came down. Under the rubble: a rusted signal-horn, mouthpiece worn bright from use. He was calling someone, at the end. The record doesn't say if anyone came.",
			"variants": [
		...
		"t_blow": {"who": "Narrator", "text": "The note comes out cracked, then enormous, and it rolls east over the marsh and the wastes and takes sixty years of waiting with it. Silence. Then, from far past the keep, where the storm never quite leaves the horizon: one long note back. The same call. An answer six decades late, or an acknowledgment. Somewhere, a watch that never ended has changed hands.", "next": ""},
		...
		"t_sign": {"who": "Narrator", "text": "The wind takes its slow inventory of the broken stones and leaves the count unchanged. The burn-shadow keeps its post.",
			"choices": [
				{"text": "Cut the hunter's sign into the fallen masonry, clear of the burn-shadow.",
					"req_flag": "sq_on_hunters_rounds", "req_not_flag": "hunter_mark_tower",
					"flags": {"hunter_mark_tower": true}, "next": "t_mark"},
				{"text": "Let the tower keep its one mark tonight. Come back with a steadier hand.",
					"req_flag": "sq_on_hunters_rounds", "req_not_flag": "hunter_mark_tower", "next": ""},
			],
			"next": ""},
