...
const CLASSES := {
	...
	"assassin": {
		...
		"abilities": {
			...
			"ult": {"name": "Death Mark",   "cd": 30.0, "mp": 0,  "dmg": {"coeff": 1.3, "base": 0.0, "type": "true"}, "riders": {"amp": 0.5, "amp_secs": 5, "iframe": 0.8, "notes": ["Executes low HP"]}, "desc": "Mark your prey with the X: two shadows converge THROUGH it, then blink toward its far side for the killing stab, stopping short if the landing is blocked. TRUE damage; the marked target takes +50% damage for 5s. You are UNTOUCHABLE for 0.8s through the execution. FIXED 30s cooldown — no haste can hurry the execution."},
		},
	},
	"paladin": {
...
const ABILITY_THEMES := {
	"warrior": {
		...
		"a2": {
			...
			"bulwark": {"desc": "Shield high: charge THROUGH the danger — even a near pass clips them, and every enemy rammed heals you and fortifies your guard.",
				"fx": {"heal": 0.03, "guard_buff": 40.0, "graze_heal": 1}},
	...
	"assassin": {
		"a1": {
			...
			"shadow": {"desc": "Strike from the dark: your MARKED prey (Death Mark / Exposed) always crits, and the blade flows — +5% combo.",
				"fx": {"combo": 0.05, "marked_crit": 1}},
		...
		"a2": {
			...
			"shadow": {"desc": "Phantom step: dash further — and ANY kill within 2s refunds most of the cooldown. The best mobility in the kit.",
				"fx": {"dash_mult": 1.35, "kill_refund": 0.7}},
		...
		"a3": {
			...
			"shadow": {"desc": "FIVE blades in a tight converging arc, all hungry for weak points.",
				"fx": {"knives": 5, "spread": 0.15, "crit_bonus": 0.10}},
		...
		"ult": {
			"poison": {"desc": "The mark rots: injects a massive 5s venom that eats the target alive.",
				"fx": {"mark_dot": 0.6}},
			"shadow": {"desc": "Executioner: if the execution leaves them under 30% HP, a TRUE-damage finisher lands. (The mark's vuln is a leaner +40% — shadow's edge is the guaranteed crits, not the amp.)",
				"fx": {"execute": 2.0, "vuln": 0.40}},
	...
	"paladin": {
		"a1": {
			...
			"aegis": {"desc": "Strike with the shield edge: blows stagger and harden your guard.",
				"fx": {"stun_chance": 0.20, "guard_buff": 25.0}},
