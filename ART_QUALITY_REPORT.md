...
**Environment taxonomy correction (2026-07-28, supersedes the first anti-litter interpretation).** The first fix prevented duplicate landmark sprites but still placed the same terrain ecology composition in every matching room and incorrectly treated accents as singletons. The owner clarified three separate tiers:
- **Landmarks:** every procedural live/placeholder terrain resolves to a roster of at least five candidates. The room selects exactly one seeded candidate; `ecology`, terrain structures, and family candidates are mutually exclusive rather than additive. Rooms sharing a terrain draw from a seeded shuffle without replacement until the roster is exhausted. The graveyard therefore rotates among its memorial, mausoleum, crypt, angel, and mourner instead of stamping `grave_memorial` everywhere.
- **Accents:** every terrain resolves to at least five kinds. Multiple kinds may coexist, each as one local group. Group counts use per-kind normalized bell curves: crystals, giant fungi, and large-tree stands peak at three; lone fixtures such as coffins/signs peak at one, with two rare.
- **Props:** repeatable `obstacles` + `decor` remain freely scatterable and every procedural terrain has at least five kinds. Thin legacy previews `ph_hall` and `ph_fae` gained one prop kind each to satisfy the contract.
- Signature landmark sprites remain reserved across placement channels. Authored duplicate silhouettes were also varied: `storm_array` uses one charged stone plus an ordinary rock, and `mausoleum` pairs a mourner with an angel.
- Regression coverage now audits the 5/5/5 roster minimum, bell-curve modes, landmark/prop separation, authored-composite uniqueness, and exactly one runtime landmark node per procedural room on desktop and mobile.

**Kinetic-prop and physical-footprint correction (2026-07-28).** A garden fountain exposed two systemic defects: its `fountain_flow_anim.png` was literally four opaque blue squares, and its ~150-165px basin used one 30px-radius collider. The fountain itself already contained good authored water, so the replacement animates only the cascade/ripple layer rather than redrawing the stone shell.

- **Generated/replaced raster assets:** `fountain_flow.png` + `fountain_flow_anim.png` were replaced; `spore_puff(.png/_anim.png)`, `void_energy(.png/_anim.png)`, and `storm_arcs(.png/_anim.png)` were added. Each strip has four square frames and a transparent background.
- **Shared motion seam:** `Terrains.PROP_MOTION` attaches a local animated child to stable authored shells. Spore vents puff, void rifts and the previously static Waking Depths gate coil/arc, storm conductors crawl with electricity, and furnace/brazier families animate their fire. This works through obstacle, accent, structure-base and structure-part placement.
- **Fountain contract:** `town_fountain`, raw `garden_fountain`, and `holy_sanctum` now use the same visible-water behavior. Each basin uses two overlapping broad circle lobes (rather than one tiny center circle), matching the full elliptical stone rim and reaching its back edge.
- **General collision audit:** the fixed 11px scenery circle is replaced by `Balance.SCENERY_COLLIDER_RADIUS` for authored high-resolution solids. Rocks, boulders, logs, shrines, furnaces, crystals, statues and similar props now block their visible ground footprint; trees deliberately retain trunk-sized collision rather than blocking their canopies.
- **Kinetic landmark definitions:** standalone magma furnaces, storm conductors, void rifts and spore shrines now own explicit footprint shapes. Magma furnaces also carry animated ember smoke and positional fire audio.
- **Art direction:** built-in image generation used the existing regenerated prop as a style reference, requested stylized dark-fantasy game sprites with lightly pixel-stepped edges (not photorealism), and generated on a flat chroma-key field. The chroma field was removed locally, alpha was validated, and the four frames were assembled into the engine's horizontal-strip format.
- **Validation:** desktop compile gate and quick suite pass; regression tests require all four motion families, local-overlay attachment, three fountain aliases with animated water + two broad basin lobes, and non-legacy collider radii for representative large solids. A two-frame in-engine visual capture confirmed changing water, spores, void energy and lightning with no rectangular overlay.
