...
**Chapter 2 is the deliberate exception — the terrain tour.** Where ch3–7 each commit to one biome family (ch3 graveyard, ch4 magma, ch5 ice, ch6 bog/spore, ch7 storm/void), ch2 crosses ten terrains, roughly one per spine room (bog → storm → spore → graveyard → desert → ice → crystal → holy → void). This is the "later chapters blend" beat pulled early on purpose: ch2 sits on the leading edge of the Waking (the corruption cracking the settled world open), so its map has no business staying one biome — the impossible desert-beside-ice adjacency IS the Waking warping the land. Because mobs are terrain-keyed, this is also why ch2 fields ~14 enemy types against ~7 in every other chapter (its side rooms attach by terrain, so the variety compounds), while carrying the *fewest* total spawns (94) — variety, not volume. Each biome previews the Act-1 chapter that later makes it home, so the tour doubles as onboarding. The accepted cost: ch2 has no single signature biome; its identity is the tour itself.
...
### Difficulty tiers / NG+ — SHIPPED 2026-07-24
...
### The Capital — Crownfall as the home base — SHIPPED 2026-07-25
(spec + judgment log: PROPOSALS/CAPITAL_REWORK.md; round narrative: BALANCE_HISTORY top entry.) **Nine rooms, 3×3** (gen_capital.py): Crown Plaza — the only grand room — holds every everyday service ringed around the fountain (forge, lapidary, vault coffer, bazaar + wardrobe stalls, mail, alms, city map); the branches are the Wayfinder Sanctum (3 mode portals), Grand Archive (codex/journal/records), Ashen Tankard (party lobby + hearth + alembic), Emberward Gate, and one room per faction ward. **One access point per function** (generator-enforced): services with favor attribution live on NPCs (`forge`/`lapidary`/`drill` hub actions, one-time greet convos), impersonal functions on props; landmarks may be pure scenery. **The capital is THE shop**: chapter start rooms open merchant-less; the Crown Bazaar restocks at dawn (character-saved shelf, stocked + priced for the road ahead via `shop_chapter()`, no gamble); mid-run merchants charge posted road prices (+10–20% seeded bell curve). **Bench trades**: reforge (Petra) and all gem work incl. equipped-first synthesize (the Lapidary) happen only in Crownfall — bag-only gem merges work anywhere; the stash lost its HUD icon (the plaza vault IS the stash). **NPC favor v1** (Petra/Lapidary, per-character, never resets): bench spend + intro-quest turn-ins, shard-scaled gains, posted −2/−4/−6% bench discounts at 25/75/150. **Onboarding**: first ch1 clear routes solo heroes to Crownfall (co-op marks the gate); ❢-marked intro quests (seat a gem / temper a piece / commit a talent point) turn in at their giver for gold + favor. **Chapter end = way-gates, not a wall**: the victory card dismisses (ENTER/tap) into the live world where three gates stand — Crownfall / fresh pass / onward — solo acts direct, a partied host rides the MP-20 proposal + reprise picker; the card keeps grades/PBs. Boss fights open on a splash-art flash (`hud._boss_splash_intro`, visual-only, no-pause-safe). **Roads stay walkable (2026-08-17, CAPITAL_REWORK §6c):** in every capital room every door reaches every other door on foot, and the north road passes THROUGH the city-edge arcade's arch (its base strip carries an unscaled `lane_gap`; a hall on the road leaves a corridor behind it) — suite-guarded by a hero-radius flood fill with a negative control; capital rooms carry NO terrain scatter (the accent family pool is gated off `capital_*`, like the landmark draw).
...
- Fixed chapter XP: only authored packs + bosses pay; event/summon spawns pay zero. **Replay XP rides a PARITY CEILING (2026-08-17, the "grey mob" rule — PROPOSALS/REPLAY_XP.md; supersedes round 3's flat "completed chapters pay no XP"):** a completed chapter pays FULL XP while the hero is below its parity level (its finale boss's level — ch1 10 · ch2 16 · ch3 22 · ch4 28 · ch5 33 · ch6 37 · ch7 41, `Story.chapter_parity_level`), tapers to zero across parity..parity+`REPLAY_XP_OVER` (3), and never pays at/after `ACT_XP_CAP` (40 — the tree fills there; Mastery owns 41+ later). So a stuck hero can go back and grind the chapter behind them until it goes grey, and the frontier out-pays it in XP as well as gold; the round-3 exploit stays bounded by construction (ch1 → Lv13, forever). Uncompleted chapters pay as authored on every attempt; NG+ tiers pay zero. Legibility: the replay picker prints "XP pays until Lv N" per chapter, the results card carries an XP line, and the first capped kill of a run says "outgrown this road" once. New-chapter rule: sum authored pack XP through the 30+22·lvl curve; land each boss room at boss level (full clear tracks parity ±1).
- Rewards LINEAR per level. **Chapter loot BANDS (2026-07-09, replaces the act-keyed ceilings):** every gear/bag drop is a weighted roll from the chapter's table (`Balance.CHAPTER_GEAR_WEIGHTS` general / `CHAPTER_BOSS_WEIGHTS` boss), no more roll-high-then-clamp. Tiers phase in/out on a sliding window — **F ch1-3 · E ch2-4 · D ch3-7 · C ch4-11 · B ch5-∞ · A ch6-∞ · S ch12-∞**. GENERAL faucets (mobs/chests/shop/spoils/gamble) skew low-mid; the BOSS channel reaches the ceiling (~1/3 per boss, `BOSS_GEAR_CHANCE`); bags follow the boss table everywhere (drop/elite/shop). `loot_cap()` = the chapter general-band ceiling. Regular gems drop **ch4+**, special gems **ch6+** (`regular_/special_gems_drop`). Gem slots F–D = 0; **C = 1 regular socket, gem cap Lv2** (2026-07-09 — sockets extend down a tier so ch4's first gem drops land on ch4's C-band gear). Per-chapter weights hand-authored ch1-12 (ch12 = the S-band table, authored 2026-07-24 for the NG+ tier shift); ch13+ set when built. **Econ re-measure PENDING** (`CHAPTER_ECON` still reflects the old tiers).
...
- **Gear-power curve (2026-07-28, replaces the 2026-07-27 gear-ceiling ramp exponent):** UPSCALED monsters (bosses AND far-field mobs) scale HP by the MEASURED expected-gear-power curve — `Balance.GEAR_POWER_ANCHORS` (level -> dps-bench-measured damage multiplier of the kit the game fields by then, vs the at-level A-grade baseline), geometric interpolation between anchors, flat 1.0 below L50 (uniques haven't stacked; aligns with the NG+ unlock) and FLAT past the cap (gear stops growing there — deeper-content bite belongs to the growth dials, not this curve). One call site (`enemy_stats_at`, codex-honest); ratio taken from max(anchor, L50) so native-band stats stay exactly as authored; at-level fights untouched (L42 A-gear finale re-measured at the ~40s budget). HP only — boss DAMAGE stays on the flat dial (L100 finale patterns measured 30-83% max HP per hit on perfect kits). Calibration pin: **#1 sustained dps (perfect kit + its best SET) kills the L100 Act-1 finale in the 30-40s proxy band** (the TTK-band rule below owns this pin; an earlier ~60s figure here was stale); re-measure an anchor (dps_bench --ttk [--setprof]) whenever a gear phase lands.
- 1 attribute point/level; attribute points buy substats 1:1 (combo deliberately NOT purchasable).
- **Talent tree (levels 1–40):** row-based, 3 theme columns × 4 rows. The starter row is always open; later rows open at levels 20/30/40 OR when the immediately preceding row is full. Each row and authored cell holds up to 10 points. 1 talent point/level and **points == level**, so the tree fills EXACTLY at 40 — the close of Act 1, with no points wasted early or late. Per-cell values are HALF the old 5-point tuning so a maxed cell lands the same total; specials/speed can never be bought here. Past 40 the tree is DONE — see the Mastery layer below.
...
- **Boss TTK band (owner standing rule, 2026-07-28 — supersedes the 25s/30s/40s ladder):** EVERY boss, fought at its corresponding level with era-appropriate gear, dies to the DPS KING (the assassin, per the measured board) in **40s minimum to 90s maximum**; deviation beyond 5s outside the band (<35s or >95s) is unacceptable. The band governs the whole boss health curve, chapter-1 opener through endgame — a fresh ch1 character sub-40s is as much a bug as an at-level finale sub-40s. Slower classes may exceed 90s (defensive identities buy survival; the band is measured on the king). ENDGAME PROXY (until Act 2+ bosses exist as references): the upscaled Act-1 finale vs the L100 BiS+godroll assassin pins at **30-40s** — the perfect-kit CEILING deliberately undercuts the band floor so realistic L100 kits land inside 40-90s; future tuning owns the upper levels when real endgame bosses land. Rungs are re-measured with dps_bench --ttk (era kits: level-budget talents, act-era grade); band rung provenance lives in BALANCE_HISTORY.md. Boss damage growth stays pinned to the player curve (`BOSS_DMG_GROWTH`, 0.015) with a constant 1.2 skill tilt. Retune pools whenever player power moves.
- **Melee basic swings ALTERNATE (owner standing rule, 2026-08-16):** a melee hero's basic attack never replays one identical motion — cast by cast it flips between two authored swings (Cleave: overhead chop ↔ level sweep; Judgment: overhead slam ↔ sideways swing; Stab: lunge ↔ double-dagger cross-slash) via the `attackb` clip (`Art.HERO_CLIP_FILES`, `player_core._alt_basic_clip`). Art-driven: a body without an `attackb` strip (skins today) keeps its single swing. Each alt swing's hit stays on ITS contact frame (`Balance.ALT_SWING_DELAY`, the same rule as `Skins.SWING`). A new melee class or skin ships both swings, or the primary alone — never a second swing without its measured contact time.
- **Boss threat-speed grammar (2026-07-22):** AIMED boss projectiles (volleys/fans at a target) fly at `Boss._aimed_speed()` — trash-bolt speed (420) at the ch1 door laddering to 480 by the finale (`BOSS_BOLT_AIMED_*`) — so dodging one is an INPUT, never a byproduct of walking (player walk is 248–275; the old 270–340 bolts couldn't catch a moving player inside the 2.5s projectile life). RADIAL rings stay slow-and-readable walls dodged through the gaps, but never slower than the walk (`BOSS_BOLT_RING` 300 — a faster wall is unplayable chip; a slower one is auto-dodged by fleeing). STANDING boss melee plays the swing first and lands damage on the contact frame with a reach RE-CHECK (`Boss._melee_swing`, +`BOSS_MELEE_CONTACT_GRACE` — stepping out during the visible wind-up IS the dodge); CHARGE/pounce contact hits stay instant (the charge telegraph was their wind-up). Teleport gap-closers land OUTSIDE contact reach (the Varo rule: reposition, never ambush — `ECHO_BLINK_LAND`).
...
- **Profile SETS** (2026-07-28, replaces the retired S-legendary class set): 30 sets = 6 classes × 5 profiles (ward/guard/finesse/aggressor/bulwark), six gear slots each, membership by VERB family (`Items.set_profile_of`), 2pc/4pc/6pc tiers in `Balance.UNIQ_SETS` (4pc = the lens, 6pc = one standing capstone; weapons never count). **Completion rule (owner, 2026-07-28): higher completion must out-pay partial — for a class's premier damage profile the bench-measured value must ESCALATE 2pc → 4pc → 6pc, each tier's increment out-paying the named-unique passive the extra slot displaces (never price a set's identity into its 2pc). Verified per class with `dps_bench --setprof` whenever set numbers move.**
...
6. **Deferred:** transmog/cosmetics (long-tail chase, but wants an art pipeline we don't have yet); pity timers (the gem/boss-gem curves already do implicit bad-luck protection); **Story Theatre** — replaying chapter opening scenes from the codex, choices offered again but NEVER saved (owner-wanted 2026-08-15; it is a state-setter sandbox job — spec + build sketch in `PROPOSALS/STORY_THEATRE.md`; the codex's *Story so far* shelf re-READS the archive meanwhile).
...
- **XP = story currency.** Fixed totals, parity-budgeted (rounds 4/5); paid in full on the first run and, since 2026-08-17, on replays only UNDER the chapter's parity ceiling (the grey-mob rule above) — so the story can't be farmed and farming can outlevel the story by at most `REPLAY_XP_OVER` levels of the chapter behind you, never the one ahead. Quests/spine pay XP + beats, NEVER repeatable income beyond that ceiling.
- **Gold = the farm currency.** Linear per level, so the frontier always out-pays: measured replay rates climb ~39 → 71 → 105 → 181 g/min across ch1→ch7 (audit 2026-07-24; ch1 fell from the 07-06 audit's ~47 with the band-table change — the F-only chest band sells for less). Replay naturally earns ~1.3-1.5× first-run g/min (same faucets, shorter run) — farming is VIABLE by default; the first run pays in XP + the first-clear beat instead.
...
- **Renown = the status currency (2026-07-24).** Account-wide, fed only by the collected faucets above plus record pushes and tier first-clears, spent only on cosmetics + a capped weekly consumable cache — a horizontal track that can't leak into power (see "Renown & the Wardrobe").
...
**Open items:** (1) **Ch2 reward hole — CLOSED 2026-07-26 by graph retrofit** (CH2_RETROFIT_TASKS, round entry at the top of BALANCE_HISTORY): ch2 was the last LEGACY STRIP — no `spine`, so it laid out as a fixed west→east chain and, having no side-attach pass, could carry none of the room types the premium path seeds off. It is now a spine chapter of **20 rooms** in the ch3 shape (9 combat / 3 boss / 2 social / 2 dead-end / 2 resonance / 1 merchant / 1 safe), with the ten authored rooms held at spine indices 0–9 and side rooms appended after. Replay **35.6 → 47.7 g/min** (like-for-like, both on the corrected audit) on a fixed chapter XP budget (2926 → 2932, +0.2%); caches 0 → 29g, elites 1.1 → 2.6/run, i.e. every *structural* faucet now sits at ch3 parity. The residual gap to ch3's 68.4 is **pure level curve** (mobs 924 vs 1262, bosses 224 vs 457) and was deliberately not forced — see the round entry. ch2 also inherits Waking Incursions, which required a spine. (2) The 15–25% first-clear share and the 65% gem-quality cap are first guesses — confirm in the difficulty-tuning playtest. (3) Replay time assumption (32 min vs 55 first) is the audit's weakest estimate — check against real replays; the ch2 retrofit sharpened this one, since the flat 32 min was measuring a 10-room chapter against 21-room ones.
...
2. Elites/affixes + difficulty tiers (instant replay hours) — **elites DONE (round 6); tiers SHIPPED 2026-07-24 (see "Difficulty tiers / NG+")**
...
- **Track B — ambient life. FIRST WAVE 2026-07-06; critters REBUILT 2026-08-17:** `ambience.gd` (Ambience.populate keyed off terrain, spawned with the scenery, zero gameplay weight). Critters are now **six Codex-animated species** (bird/crow/butterfly/bat/dragonfly/hawk) that FLY — flit through the air + sky-crossing soar — spawn in **flocks/clumps** (not lone strays), and FORAGE (birds land, hop/peck/stand on a folded-wing perched strip, then flush when you close in). Every non-boss terrain has movement now (bats over the keep, wisps in the crystal caverns, drifting ash + heat-haze on magma, fog banks on graveyard/void, will-o'-wisps + dragonflies + water-plops over marsh/bog). Painterly→pixelate art via `tools/art/install_critter.py`; strips are `critter_<kind>.png` (square frames, count = width/height). Plus foliage **wind-sway shader** (`Art.wind_material()`, one shared material, phase from world position) on trees/flowers/mushrooms; **per-biome ambient audio beds** (`Sfx.make_ambient`: birds/wind/cold/crickets/drone, seamless 8s loops; `game.refresh_ambience()` per-frame hook switches on terrain and also sets light_mult). Still open: chimney smoke, cat/chickens, snow footprints, rustle-on-walkthrough, dash dust.
...
- **Track D — environment art seams. SHIPPED 2026-07-18.** The three code blockers that kept whole pack sheets frozen out are gone; each is a pure drop-in seam, so unlocking is now mechanical (drop the PNG, no code):
  1. **Ground PNG tilesets** (`Art.ground` + `_ground_tileset`): drop `assets/sprites/ground_<kind>.png` — a single seamless square tile OR a grid of square variation cells (strip/column/NxM) — and that ground KIND tiles the pack art instead of the procedural palette, picking a seeded variation cell per 16px tile. The procedural noise/macro passes skip a tiled band; roads stay honest. No override → the biome is byte-identical to before, so shipping terrains are untouched until art lands. (~10 pack ground sheets unlocked.)
  2. **Composite structures + wall decals** (`game_world._add_structure` + `Terrains.STRUCTURES`): a structure is several sprites y-sorted as ONE base-anchored body, a MULTI-shape footprint collider (an L-ruin/gate needs more than the one circle `_add_obstacle` gives), and non-colliding wall decals (banners, torches) that can animate + carry a point light. Zones/terrains opt in via a `structures` list, like `buildings`; an unlisted name degrades to a single base sprite + footprint rect. Catalog seeded from existing art (`ruined_gate`/`watch_brazier`/`old_well`/`signal_fire`), previewable in the dev-only `ph_ruins` terrain. (~8 pack structure kits unlocked.)
...
### Standing rules
- **Environment taxonomy + distributions (env doctrine 2026-07-17; clarified 2026-07-28).** Every procedural terrain exposes at least five kinds in each tier. **Landmarks** are major, terrain-specific compositions: choose exactly one seeded candidate per room from a roster of at least five; candidates are mutually exclusive, never additive, and are shuffled without replacement across rooms sharing that terrain until the roster is exhausted. **Accents** are distinct local features: several kinds may coexist, but each kind appears as one group whose count is sampled from its authored normal distribution (`peak`, `sigma`, `max`)—for example, a large-crystal or giant-mushroom group peaks at three, while a coffin/sign peaks at one and a second is rare. **Props** are the repeatable `obstacles` + `decor` ecology—trees, ordinary rocks, grass, flowers, and similar scatter—with at least five kinds per terrain. Natural clumps, room density, mob size, and accent group sizes use probability curves rather than flat `randi_range` rolls. Encoded in `Terrains.LANDMARK_POOLS`, `Terrains.ACCENT_PROFILES`, and `Balance.SCENERY_*`.
- FX doctrine applies to all of this: nothing ships until SEEN in a `shot_kit.gd` screenshot.
- **FX layering (owner ruling 2026-08-15): actors draw ABOVE effects.** Bosses, mobs, heroes and NPCs are never covered by an impact, cloud or ground burst — the effect's body sits UNDER the actors (z below 0), and whatever must read as engulfing them (a fireball dome, a gas veil, a splash) is a translucent copy over them (~30–50% alpha), never opaque. `player_combat._fx_flash` implements this as `{"z": -1, "ghost_over": 0.45}` (full-alpha body under, ghost over); `_mist` does it with a body-under + veil-over split. Short thin marks (slash lines, cut flashes, rings, damage text) may stay above. **Ground rings and floor bursts get NO ghost at all** (owner 2026-08-16, Consecration): a ring on the ground is floor — characters stand strictly on top of it, nothing translucent over them. The see-through copy is only for effects with volume (a flame dome, gas, a blood eruption, a void blast).
- Ambient life carries zero gameplay weight (no XP, no drops, no aggro) — critters are scenery that reacts, not entities.
- **PLACEMENT IS EXCLUSIONARY — nothing spawns on a surface it can't belong on (owner 2026-08-17, "logical inconsistency"):** a prop/landmark/accent must never land ON the river/water, and a ground critter must never walk/forage on water. Whatever a thing is placed on is *controlled*. The scatter reserves off the road/door lanes AND the river channel (`_spawn_scenery` decides the river FIRST, then adds its band to `reserved`; birds carry a `no_land` rect and stay airborne over water). The bug this fixes: the river used to be rolled AFTER the props, so trees dropped onto the Greyrun. Rule for any new placeable (prop, critter, hazard, FX): decide the exclusion zones first, then place — never the reverse. This is the composition half of MIMIC NATURE.
- **MIMIC NATURE — the overarching world doctrine (owner 2026-08-17): always ask "does it feel real enough?"** Nature is *composed*, not sprinkled, and *appearance is controlled by rarity*: a **LANDMARK is singular** — it's a landmark, so one per room, never spammed (this is why the env taxonomy above picks exactly one, shuffled without replacement, and `UNIQUE_PROP_NAMES` caps signature silhouettes at a single copy); **ACCENTS may recur but their count is governed** by a normal curve (a lone sign/coffin peaks at 1, a crystal stand or fungus cluster at ~3 — `ACCENT_PROFILES` peak/sigma/max), never a flat spray; repeatable **PROPS** carry the in-between. Ambient **LIFE obeys the same law** — flocks and clumps not lone strays, social species group while solitary raptors fly alone, birds land and forage rather than hover forever. It is ONE principle with two encodings: the Environment taxonomy above (`Terrains.LANDMARK_POOLS` / `ACCENT_PROFILES` / `UNIQUE_PROP_NAMES`) for *composition*, `ambience.gd` for *movement*. Apply the realism test to every new terrain, prop, accent, and ambient detail before it ships.
...
never change damage, damage ticks, cast/release/recovery or hit timing, targeting,
range, projectile physics, collision, cooldowns, ability behavior, or progression. **Elite**
...
distinct silhouettes on every ability, and a staged showstopper ult. Its
S-weapon awakening evolves the **character sprite only** into a cooler, more
dangerous form. It must be a reworked sprite—not a palette/band recolour—and
never adds, replaces, or modifies the mythic ult or ability FX. Golden Ronin is the elite
bar; Phantom is the mythic bar. Colour swaps and generic-FX substitutions do
not satisfy either tier on their own.

The concrete animation boards, asset rules, review captures and completion
gates live in `SKIN_ANIMATION_BIBLE.md`; that document controls implementation.
...
motifs are focused martial/caster variants; Stormforged and Eldritch Warlock
...
|---|---|---|---|
| Warrior | **Dreadknight: Blood Oath.** Jagged crimson soul-cuts; Charge leaves a banner-like wake of harvested spirits; Whirlwind carries spectral greatswords. **Ult:** he raises the sword in an oath stance while a bleeding war-standard unfurls behind him; the brand then races up the blade and into his armour as Berserk ignites. | **Stormforged: Eye of the Tempest.** Static crackles from movement, strikes discharge into victims, and Charge is a lightning-body bolt. **Ult:** an eye of storm opens overhead; three bolts orbit the warrior and feed into the armour as storm-light locks in for Berserk. **Awakened sprite:** blackened storm plate split by cyan lightning seams; a featureless white-lit helm beneath longer forked horns; a torn lightning-edged mantle; and a wider forked lightning blade. The sprite itself carries the arcs, like Phantom's awakened wisps. | Keep — oathbound knight is a focused elite; living storm avatar is mythic. |
| Archer | **Frostfall Ranger: Winter Hunt.** Faceted ice arrows leave small frost blooms; Multishot is an icicle fan; Tumble sheds snow and broken rime. **Ult:** a frost-moon sigil forms above the archer before the hail-arrow storm begins. | **Voidwraith: Eclipse Hunt.** Void feathers/motes accompany movement; arrows bend into barbed dark streaks; Tumble folds through a small rift. **Ult:** a black moon opens above the arena, silhouettes a volley, and releases the rain of arrows from its centre. **Awakened sprite:** the readable face becomes a dark void with two sharp violet lights; the hood and cloak grow into a longer feather-torn eclipse mantle; the bow becomes a large asymmetric crescent with an energy string; and embedded violet wisps frame the silhouette. | Keep — winter hunter is elite; eclipse predator changes the whole battlefield's read. |
| Mage | **No elite assigned yet.** The former Void Weaver concept and exact character sprite have moved to the Warlock mythic. | **Crystal Archmage: Prismatic Cataclysm.** Walking sheds refracting facets; bolts split light; Blink shatters and reassembles the body; Nova grows crystal spokes. **Ult:** orbiting prism glyphs focus into a crystal monolith that fractures into luminous shards on impact. **Awakened sprite:** replace the plain robe silhouette with a crystal regalia mantle—large faceted shoulder shards, a split translucent crystal hem, a brighter gem-staff head, and an icy white eye/crown treatment. Static prismatic shards are drawn behind the body in the sprite frames, never spawned as new FX. | Keep Crystal Archmage as the sole completed Mage skin until a new elite identity is authored. |
| Paladin | **Eclipse Knight: Last Corona.** Every gold beat is opposed by a black inner disc: crescent Judgment, black-sun Consecration, umbral Aegis. **Ult:** an eclipse crosses the verdict zone and a corona hammer falls through the dark. | **Fallen Arbiter: Cold Verdict.** Frost motes and pale wing-shadow follow movement; attacks leave judgment seals on victims; Aegis is an angular ice-law sigil. **Ult:** spectral court seals condemn the field, then an immense white verdict blade descends. **Awakened sprite:** replace the simple colour shift with a faceless cold-white helm, a broken floating crown/halo, broadened wings whose outer feathers resolve into pale spectral blades, and a white-blue verdict weapon. The body stays recognizably armoured and winged, but reads as an angelic executioner rather than a dark knight. | Keep — an eclipsed knight is elite; an inhuman executioner of a celestial court is mythic. |
| Warlock | **Hellfire Inquisitor: Brandbearer.** Retain the existing fire shadowbolt; add ember footfalls, hot-iron Hex marks, and a brazier-like Dark Pact around it. **Arcane Warlock:** the former green Eldritch Herald is renamed and retained as an elite without further animation work. | **Eldritch Warlock: Eyes Between the Seams.** Uses the exact former Void Weaver character sprite. Shadowbolt calls a side-looking woven eye into a clear pocket and fires a tiny violet needle with a full-path trail. A four-wing eye familiar follows the caster. Hex places a winged watcher above each cursed enemy and fires a runtime thread into it on every real DoT tick. Dark Pact inherits the stitched circle/unravel; Void Rift lights its exact circle and erupts in vertical threads on the unchanged burst beat. | Keep — no duplicated character art and no skin-specific gameplay timing, range, damage or tick changes. |

**Production gate:** before art or FX work begins on a skin, its brief must
identify its movement accent, all four ability silhouettes, and its ult timeline.
For mythics, separately identify a sprite-only awakened-form concept with a
stronger face/head treatment, weapon or focus, costume/cape silhouette, and
embedded energy detail—visible at gameplay scale without relying on a hue
shift. A skin cannot be called complete
