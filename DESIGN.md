...
- Rewards LINEAR per level. Act loot ceilings (2026-07-06): **Act 1 = F→B, Act 2 introduces A, Act 3 owns S** (`Balance.ACT_LOOT_CAP`, clamped centrally in `loot_cap()` over authored caps — ch1's authored C stays). Gem slots F–C = 0.
- **Special stats are GEM-ONLY (2026-07-06):** Haste, Lifesteal, Combo and Greed never roll on gear — not in the sub pool, not in shape personalities (charm main moved Haste→Crit; Bow→DEX, Wand combo→crit, Tome lifesteal→MP, Striders→EVA). Gems are the gateway to off-build stats: **at most ONE special gem per item**, and recalc HARD-caps the totals — Haste 40% / Lifesteal 35% / Combo 30% — regardless of source. S legendaries keep their small authored specials (quest-gated identity; the caps guard the ceiling). Late game may lift caps a notch by level (~L80) — noted, NOT built. Supersedes round 43's B-gate.
