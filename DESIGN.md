...
- **Gear-ceiling ramp (2026-07-27, replaces the gem ramp — which had been 0.0 since 2026-07-09):** UPSCALED monsters (bosses AND far-field mobs) gain +4.85%/level compounding HP for levels above max(L50, their anchor) (`GEAR_RAMP_*`, inside `enemy_stats_at` — codex-honest): past L50 the player's kit detaches from the base ~5.5%/level curve (S-band gear, named-unique passives, Lv8-10 gems, deep smith/reforge stack multiplicatively), so flat player-tracking growth under-budgets endgame pools. HP only — boss DAMAGE stays on the flat dial (the unique gear is all offense; L100 finale patterns measured 30-83% max HP per hit on perfect kits). Calibration pin: **#1 sustained dps (perfect kit) kills the L100 Act-1 finale in ~60s**. Anchor stats stay exactly as authored; at-level fights untouched (L42 A-gear finale re-measured at the ~40s budget). Start=50 aligns with the post-Act-3 NG+ unlock.
