...
static func _notes_wayfinder(m: Menus, list: VBoxContainer) -> void:
	...
	for pair in [
		...
		["Earn a shorter road", "Some Chapter 1 layouts, including weekly runs, have a barred optional shortcut. Walk the original road to the latch on its far side and secure that room, then work the latch. The passage opens for the whole party for this run. It never replaces a story gate, and some layouts have no safe place for one."],
...
static func _notes_combat(m: Menus, list: VBoxContainer) -> void:
	for pair in [
		...
		["Learn from a fall", "Pause and open Combat report to inspect the final blows of your last fall, or damage from the last 12 seconds of gameplay. Reading a paused solo menu keeps those wounds in view; co-op menus leave the world and its history clock running. It records health lost after shields and mitigation, never overkill. Recovery keeps the last fall for the current character session. Environmental damage may not have a named attacker."],
