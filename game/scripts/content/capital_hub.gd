...
const CONVOS := {
	...
	"cap_kesh": {"start": "a", "nodes": {"a": {"who": "Herbalist Kesh", "text": "Cures for the ward, reagents for the Alembic across the plaza. I keep the Alchemist's bench \u2014 charms and gloves worked from bone and cloth. Here to craft, or after the day's leaves?", "choices": [{"text": "Open the craft bench  (Professions)", "hub_action": "professions", "next": "craft"}, {"text": "\"Tell me about the gathering.\"", "next": "talk"}, {"text": "(Leave)", "next": ""}]}, "craft": {"who": "Herbalist Kesh", "text": "Lock a trade at the bench \u2014 only your own trade's work will take. Mastery keeps, though, whatever you swap to.", "next": ""}, "talk": {"who": "Herbalist Kesh", "text": "I set the day's gathering. Bring back the right leaves and no one dies of the wrong ones.", "next": ""}}},
}
...
static func selftest(_game: Node2D) -> String:
	...
	var coords := {}
	var known_actions := ["codex", "daily", "drill", "forge", "guild", "journal", "lapidary", "mail", "map", "portal_crucible", "portal_depths", "portal_story", "potions", "professions", "records", "vault", "wardrobe"]
	for z in ch["zones"]:
		if z.get("type", "") != "safe":
