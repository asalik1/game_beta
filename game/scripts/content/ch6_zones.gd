...
const ENEMIES := {
	...
	"bloom_acolyte": {"name": "Bloom Acolyte", "sprite": "elf_druid", "hp": 680.0, "dmg": 80.0, "speed": 100.0, "xp": 16, "gold": 28, "ranged": true, "scale": 3.2,
		...
		"lore": "Choir pilgrims who came for holy rot, saw the Blooming, and converted on the spot. Faith that flips once flips easy, and Rotmaw taught them where to aim it."},
...
const QUESTS := {
	...
	"auroch": "Cross the green fringe and put the Drowned Auroch back under",
	"gardener": "Cut through the canal and weed Rotmaw's garden",
	"curetwisted": "Reach the heart of the Bloom and face Kaethra Cure-Twisted",
...
const CONVOS := {
	...
	"ch6_accord": {"start": "a1", "nodes": {
		"a1": {"who": "Warden Palla",
			"text": "Warden Palla, Accord. The pattern-readers sent me with one sentence and I've been chewing it since: 'the Pale Root is the seal to fear.' Not the Judge, all verdicts and theater. Not the Queen, hoarding her sleepers. This one. Because fire and ice announce themselves, bearer, and growth just quietly disagrees with every boundary it meets.",
			"variants": [
				{"flag": "ch6_accord_heard", "text": "Starve it of gardeners, bearer. It can't want things on its own yet, and that's the whole window we have.", "next": ""},
			],
			"next": "a2"},
	...
	"ch6_wander_scout": {"start": "s1", "nodes": {
		"s1": {"who": "Cure-Camp Scout Renn",
			...
			"variants": [{"flag": "ch6_renn_met", "text": "Roster's still one, bearer. It'll stay one till this is done, and then I'm teaching the young ones everything she taught me, minus the last lesson. That one you're handling.", "next": ""}],
			"choices": [
