...
const ENEMIES := {
	...
	"forgemistress": {
		...
		"mechanics": [
			{"name": "The Quench",
			 ...
			 "counter": "Body-block the pool: stand on it and she quenches through you for the fight's hardest single hit (it's telegraphed, so dodge the last beat) but gains no stack. Deny the buff early; late-fight stacks turn every hit lethal."},
			{"name": "Rising Heat",
			...
			{"name": "White-Hot Slag Lobs",
			 ...
			 "counter": "Kiting no longer keeps you safe, so keep repositioning so the lobbed blast circles land behind you, not on your feet."},
		],
	},
	...
	"cinderhide": {
		...
		"mechanics": [
			...
			{"name": "Cinderhide Enrages (30%)",
			 ...
			 "counter": "The armor problem is over, so commit everything. Keep dodging the rain and finish it before the speed grinds you down."},
		],
	},
	...
	"ashpriest": {
		...
		"mechanics": [
			{"name": "The Verdict",
			 "tell": "'GUILTY: THE WEST/EAST' rings out, and half the arena washes in verdict light and detonates; below 50% it's paired, judging one half and then the other.",
			 "counter": "Stand in the un-washed half before it lands. When paired, step into the first half after it fires (now scorched and safe) so you're clear for the second wave. The callout names both halves, so trust it."},
			...
			{"name": "The Judge Attends (20%)",
			 ...
			 "counter": "The floor never fully clears now, so prioritize the verdict shelter over greed. The rain holds while a verdict is airborne, so you can cross to safety without eating both at once."},
		],
	},
}
