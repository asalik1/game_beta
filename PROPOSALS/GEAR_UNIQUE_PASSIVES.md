...
**Status: DESIGN FIRST PASS — nothing here is implemented, wired, or tuned.**
Every number in this document is a **rough placeholder, un-benchmarked**; the
dedicated dps-benchmark phase owns the real magnitudes. Scope: the 60 named
...
| Unique | Grade | Shape | Bias | Passive name | Effect | Rough magnitude (placeholder) | Off-meta? | New mechanic or reuse? | A-drawback (if any) |
|---|---|---|---|---|---|---|---|---|---|
| Siegebough | A | Warbow | atk [1] | `siegebolt` | Multishot looses siege bolts: each arrow punches through its first victim into a second. | pierce 1 extra target, Multishot only | no | Reuse — projectile `pierce` flag | Lane 1: one ability, no bonus damage |
...
| Far-Witness | A | Longbow | crit+physpen [2] | `farsight` | Arrows loosed at distant prey arrive with the full weight of the draw: long-range Quick Shots halve enemy armor. | targets ≥240px: physres treated at 50% | no | Reuse — conditional pen on the hit | Lane 1: range-gated |
| Skyline, the Arrow Before Dawn | S | Longbow | crit+physpen [2] | `herald` | The first arrow decides: your opening hit on any unwounded enemy is a guaranteed crit and marks the prey. | force_crit on full-HP targets + 20% vuln for 3s | no | Reuse — force_crit + vuln riders | — |
...
| Unique | Grade | Shape | Bias | Passive name | Effect | Rough magnitude (placeholder) | Off-meta? | New mechanic or reuse? | A-drawback (if any) |
|---|---|---|---|---|---|---|---|---|---|
| Silkneedle | A | Stiletto | physpen [1] | `gapfinder` | The needle finds the gap that's already open: Stab ignores half the armor of staggered, stunned or slowed enemies. | 50% armor ignore, CC'd targets only | no | Reuse — conditional pen on `hit_enemy` | Lane 1: needs a CC setup first |
...
| Headsman's Mercy | S | Cleaver | massive | `headsman` | Mercy is quick: Stab and Shadow Dash BEHEAD enemies below the threshold outright, and each beheading feeds the blood surge. | execute at ≤12% HP; +1.5s stab-surge extension per execution | no | Reuse-adjacent — execute logic exists (Death Mark rider / Hunger resonance) (§11.6) | — |
...
| Atlas Branch | A | Greatstaff | massive | `atlas` | The branch bears more of heaven's weight: Meteor burns truer, but the sky takes longer to answer. | Meteor true_frac 0.25→0.40; Meteor cd +6s | no | Reuse — `true_frac` + cd mult | Lane 2 BARGAIN: the cd tax |
...
| Absolution, the Last Toll | S | Oathflail | atk+crit [2] | `absolution` | Every 3rd kill rings the Last Toll: a free Consecration wave breaks from you, mending you for every enemy it touches. | free Consecration at 0.9×, heals per enemy struck (kit numbers) | no | Reuse — Consecration invoked whole; kill counter | — |
...
| Pilgrim's Burden | A | Warmaul | massive | `burden` | The burden makes the blow: Judgment strikes heavier but swings slower. | Judgment +15% damage; Judgment cd +15% | no | Reuse — ability-scoped mults | Lane 2 BARGAIN: the cd tax |
